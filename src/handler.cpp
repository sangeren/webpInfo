...
void handler::handle_get(http_request message)
{
    ...
    auto uri = message.absolute_uri().to_string();
    // cout << uri << endl;
    auto postion = uri.rfind('=');
    if (postion < 0 || postion > 1000)
    {
        message.reply(status_codes::OK);
    }

    auto webpUrl = uri.substr(postion + 1);

    // cout << webpUrl << endl;
    auto fileName = DownLoadFile(webpUrl);
    int w, h;
    GetWebpInfo(fileName, &w, &h);

    string result = std::to_string(w) + "|" + std::to_string(h);
    // cout << result << endl;
    ...
    {
        cout << "Unable to delete file " << fileName << "\n";
    }
